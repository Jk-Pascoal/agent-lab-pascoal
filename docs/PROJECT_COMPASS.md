...
- **Estado registrado em:** 2026-09-17
- **Baseline integrado na main:** 883 testes aprovados (100% GREEN)
- **Última entrega funcional integrada na main:** Ground Truth Material Rule Evaluator v1
- **Última Issue funcional integrada na main:** #129
- **Último PR funcional integrado na main:** #131
- **Último merge funcional:** `68c99bd` — Merge pull request #131
- **Última SPEC integrada na main:** `docs/specs/0129_ground_truth_material_rule_evaluator_v1.md`
...
pareamento determinístico por evaluation_case_id (paridade 1:1)
        ↓
validação relacional fail-closed por material_id
        ↓
CaseEvaluation (DecisionRecommendationCaseEvaluation / MaterialRuleCaseEvaluation)
        ↓
EvaluationReport (DecisionRecommendationEvaluationReport / MaterialRuleEvaluationReport)
        ↓
métricas determinísticas puras (accuracy / exact_match_ratio)
```
...
- garantir que as APIs de avaliação em lote consumam estritamente mapeamentos deterministicamente indexados por `evaluation_case_id`:
  ...
  ambos com paridade exata 1:1 de chaves e validação relacional de `material_id`;
- garantir imutabilidade estrita (`frozen=True, slots=True`), ordenação canônica determinística de casos por `(evaluation_case_id, ground_truth_id)`, semântica explícita de coleção vazia (`accuracy is None`, `exact_match_ratio is None`), cálculo de exatidão sem arredondamento interno e validações defensivas fail-closed;
- sustentar baseline consolidado de **883 testes aprovados** (100% GREEN) com exportações públicas no pacote raiz `src/agent_lab/__init__.py`;
- preservar separações de autoridade: `Ground Truth ≠ Prediction`, `Dataset ≠ Metric`, `Evaluation Contract ≠ Benchmark Result`, `evaluation_case_id` (identidade experimental) ≠ `material_id` (identidade da entidade de domínio);
...
- manter fora de escopo nesta etapa: Precision, Recall, F1 multiclasse, matriz de confusão, calibração de thresholds, avaliador de pares duplicados (`DuplicatePairGroundTruth`), benchmark runner, persistência em disco/banco e loaders externos.
...
MAIN INTEGRADA:
- Baseline integrado na main: 883 testes | unittest | Python 3.11.
- Última entrega funcional integrada na main: Issue #129 | Ground Truth Material Rule Evaluator v1 | PR #131 (merge 68c99bd).
- Última SPEC integrada: docs/specs/0129_ground_truth_material_rule_evaluator_v1.md (Status: IMPLEMENTED).
- Último PR funcional integrado: PR #131.
- Último merge funcional: 68c99bd.
