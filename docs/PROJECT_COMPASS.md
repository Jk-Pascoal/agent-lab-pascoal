...
- **Estado registrado em:** 2026-09-24
- **Baseline integrado na main:** 1194 testes aprovados (100% GREEN)
- **Última entrega funcional integrada na main:** Catalog CSV Ingestion Adapter v1
- **Última Issue funcional integrada na main:** #154
- **Último PR funcional integrado na main:** #156
- **Último merge funcional:** `2b8bc94` — Merge pull request #156 from Jk-Pascoal/feature/issue-154-catalog-csv-ingestion-adapter
- **Última SPEC integrada na main:** `docs/specs/0154_catalog_csv_ingestion_adapter_v1.md`
...
3. TRILHA DE HUMAN REVIEW CLAIM E RELEASE (Append-only, desacoplada)
HumanReviewClaim
        ↓
...
--------------------------------------------------------------------------------
READ PATH FACTUAL E RELEASE-AWARE (Projeção e Policy em memória — zero I/O)

A. Leitura histórica factual:
Claim facts
        ↓
...
evaluate_reviewer_claim_eligibility (Policy pura de governança em memória)
        ↓
ReviewerEligibilityDecision (ELIGIBLE / CLAIM_REQUIRED / CLAIMANT_MISMATCH / MULTIPLE_CLAIMS_CONFLICT)

B. Leitura release-aware:
Claim facts
    +
Release facts
        ↓
project_release_aware_claim_state
        ↓
ReleaseAwareClaimState (NO_CLAIM / SINGLE_CLAIM / MULTIPLE_CLAIMS)
        ↓
evaluate_release_aware_reviewer_claim_eligibility (Policy pura em memória)
        ↓
ReviewerEligibilityDecision (ELIGIBLE / CLAIM_REQUIRED / CLAIMANT_MISMATCH / MULTIPLE_CLAIMS_CONFLICT)
        ↓
RecordHumanDecisionUseCase (Fase 2: gate pré-write obrigatório com targeted reads claims → releases)
        ├─ se ELIGIBLE → autoriza dual-write sequencial não-atômico (Audit → Lifecycle)
        └─ se diferente de ELIGIBLE → interrompe com ReviewerNotEligibleError (0 writes)

--------------------------------------------------------------------------------
COMPOSIÇÃO DE APLICAÇÃO (Somente-leitura)
Workflow Lifecycle JSONL
        ↓
...
pending workflows
        *
Leitura histórica:
Claim JSONL ──► project_human_review_claim_state ──► ListPendingHumanReviewsWithClaimStateUseCase ──► PendingHumanReviewWithClaimStateItem

Leitura release-aware:
Claim JSONL + Release JSONL ──► project_release_aware_claim_state ──► ListPendingHumanReviewsWithReleaseAwareClaimStateUseCase ──► PendingHumanReviewWithReleaseAwareClaimStateItem

--------------------------------------------------------------------------------
ORQUESTRAÇÃO DE BENCHMARK (Camada de Aplicação, zero-I/O)
MaterialRecord
        ↓
...
DecisionRecommendation
        ↓
predictions[evaluation_case_id]
        ↓
...
- **`HumanReviewClaimRelease ≠ HumanReviewClaim`**
- **`CLAIMED ≠ REVIEWED`**
- **`Projection factual ≠ Policy normativa`**
- **`ReleaseAwareClaimState ≠ HumanReviewClaimState`**
- **`unreleased_claims ≠ active claim`**
- **`factual projection ≠ active claim policy`**
- **`sole_unreleased_claim ≠ active claim`**
- **`sole_unreleased_claim ≠ owner`**
- **`sole_unreleased_claim ≠ assignment`**
- **`sole_unreleased_claim ≠ winner`**
- **`sole_claim ≠ active claim`**
- **`sole_claim ≠ owner`**
- **`sole_claim ≠ assignment`**
- **`sole_claim ≠ winner`**
...
- **`evaluation_case_id` e `material_id` são identidades semanticamente independentes:** `evaluation_case_id` é identidade experimental/metrológica; `material_id` é identidade cadastral de domínio. Nenhuma é inferida ou derivada da outra; coincidência textual entre seus valores é permitida quando ambos são fornecidos explicitamente.
- **`GovernanceDecision ≠ HumanDecision`**
- **`domain contract ≠ persistence`**
- **`Serialization ≠ Repository`**
- **`Atomic Ground Truth ≠ Ground Truth Dataset`**
- **`Desserialização valida representação canônica; não repara representação não-canônica`**
- **`new typed ground truth ≠ legacy baseline`**
- **`Infrastructure parses/maps; Application coordinates; Domain validates/judges`**
- **`ADAPTER PRESERVA; DOMÍNIO JULGA`**
- **`Input Adapter ≠ Application Use Case`**
...
- **Human Review Claim Release factual (`HumanReviewClaimRelease` e `release_human_review_claim`):** representa o fato causal imutável em memória de um especialista liberar voluntariamente o claim que assumiu previamente sobre um workflow pendente; exige validação relacional de domínio pela equivalência textual estrita de Stable Principal `(specialist_id, identity_provider, identity_subject)` e não-retroatividade temporal (`released_at >= claim.claimed_at`), preservando imutabilidade estrita do workflow e do claim, com zero I/O; **`release factual ≠ active claim semantics`**: a existência do fato e da operação pura de release em domínio puro não define nem introduz semântica de claim ativo, projeção de active claim, política de active claim, ownership, assignment, winner, exclusividade, locking, lease/TTL ou SLA;
- **Princípio `Repository ≠ Projection`:** o repositório (`JsonlHumanReviewClaimRepository`, `JsonlHumanReviewClaimReleaseRepository`) preserva os fatos físicos na ordem de append; a projeção interpreta o estado factual sem criar autoridade operacional, sem eleger claim ativo e sem inventar políticas de vigência, exclusividade ou concorrência;
...
- **Release-Aware Claim State Projection (`project_release_aware_claim_state`):** interpreta o estado factual de claims à luz dos releases ocorridos (`NO_CLAIM`, `SINGLE_CLAIM`, `MULTIPLE_CLAIMS`), reconciliando os fatos imutáveis preservados em `HumanReviewClaimRepository` e `HumanReviewClaimReleaseRepository` sem mutação do histórico, sem active claim, sem ownership operacional e sem eleição de vencedor; `ReleaseAwareClaimState ≠ HumanReviewClaimState`, `unreleased_claims ≠ active claim` e `factual projection ≠ active claim policy`;
- **Release-Aware Reviewer Claim Eligibility Policy (`evaluate_release_aware_reviewer_claim_eligibility`):** governa a autoridade normativa pura em memória a partir do read-model factual `ReleaseAwareClaimState` e de `VerifiedSpecialistIdentity`, categorizando deterministicamente em `ELIGIBLE`, `CLAIM_REQUIRED`, `CLAIMANT_MISMATCH` e `MULTIPLE_CLAIMS_CONFLICT`, com base em equivalência textual exata de Principal Estável `(specialist_id, identity_provider, identity_subject)`, isolando metadados de verificação e sem presunção de active claim, owner, winner, lease, lock ou TTL;
- **Runtime Enforcement Gate (`RecordHumanDecisionUseCase`):** aplica a política de elegibilidade em tempo de execução como gate pré-write obrigatório sobre claims e releases persistidos via targeted reads (claims → releases): se `ELIGIBLE`, autoriza a gravação do dual-write sequencial não-atômico (`Audit → Lifecycle`); se diferente de `ELIGIBLE`, interrompe imediatamente com `ReviewerNotEligibleError` garantindo zero escritas (`0 writes`); falhas físicas ou corrupção de qualquer repositório propagam a exceção original de forma fail-closed (`0 writes`);
- **Application composition read-only (`ListPendingHumanReviewsWithClaimStateUseCase` e `ListPendingHumanReviewsWithReleaseAwareClaimStateUseCase`):** combinam de forma puramente determinística e somente-leitura a fila pendente com o estado factual de claims (histórico ou release-aware) em itens imutáveis `PendingHumanReviewWithClaimStateItem` e `PendingHumanReviewWithReleaseAwareClaimStateItem`, utilizando snapshot local único por repositório (eliminando N+1) e preservando a pending queue como driver set FIFO, com invariante relacional estrito `workflow.workflow_id == claim_state.workflow_id`;
...
- **Ground Truth Atomic Serialization (`ground_truth_serialization.py`):** camada pura de serialização versionada em memória (zero-I/O, determinística, fail-closed, closed-schema, `schema_version = 1`) para os três contratos atômicos de Ground Truth (`MaterialRuleGroundTruth`, `DuplicatePairGroundTruth`, `DecisionRecommendationGroundTruth`), garantindo round-trip semanticamente lossless, validação estrita de canonicalidade sem reparo silencioso, representação ISO 8601 timezone-aware, preservação de `VerifiedSpecialistIdentity`, integridade cronológica (`annotator.verified_at <= labeled_at`), ordenação relacional de pares e exportação canônica dos 9 símbolos públicos não-ambíguos com `SCHEMA_VERSION_V1` mantido module-scoped;
- **Ground Truth Benchmark Use Case (`RunDecisionRecommendationBenchmarkUseCase` e `DecisionRecommendationBenchmarkCase`):** primeiro boundary da camada de aplicação para orquestração ponta a ponta de benchmark de recomendações de governança contra Ground Truth em memória (zero-I/O); preserva a divisão `Application coordena; Pipeline produz; Evaluator mede`, valida tipos nominais e unicidade de `evaluation_case_id`, aciona o pipeline determinístico padrão (`DeterministicGovernanceValidator` → `EvidenceCollection` → `recommend_decision`) ou injetável (`RecommendationPipeline`), valida paridade relacional `prediction.material_id == case.material.material_id`, indexa `predictions[evaluation_case_id]` e delega estritamente a aferição a `evaluate_decision_recommendations`, retornando o relatório oficial `DecisionRecommendationEvaluationReport`;
- **Catalog CSV Ingestion Adapter (`load_catalog_materials`):** boundary de infraestrutura / adaptador de entrada que realiza a leitura física de catálogos operacionais em formato CSV a partir do filesystem (`str | Path`) sob parser estrito (`strict=True`), validação canônica de cabeçalho fail-closed conforme $\mathcal{C}_{\text{canonical}}$, preenchimento default com `restval=""`, rejeição de campos excedentes e mapeamento para tupla imutável de `MaterialRecord` preservando estritamente espaços em branco externos, quebras de linha internas e a ordem física original das linhas sem normalização ou `.strip()`; a validação de regras de negócio, rejeição de outer whitespace em `material_id` e unicidade catalog-wide de identificadores permanecem sob autoridade exclusiva do domínio e do caso de uso de aplicação (`DiagnoseCatalogQualityUseCase`);
...
- **Incremento integrado da Issue #77 (Pending Human Review Queue Projection v1):**
  - módulo `src/agent_lab/workflow_projection.py`: introdução da função pura `project_pending_human_review_queue(events: Sequence[WorkflowLifecycleEvent]) -> tuple[GovernanceWorkflow, ...]`;
  - pureza estrita e zero I/O: a projeção opera exclusivamente em memória sobre sequências de eventos já carregadas, sem dependência de disco ou rede;
  - semântica de tipos e validação defensiva: validação nominal da sequência de entrada (`TypeError` para não-`Sequence`) e dos elementos individuais (`ValueError` para elementos que não sejam `WorkflowOpened` ou `WorkflowConcluded`);
  - agrupamento por `workflow_id` preservando estritamente a ordem interna dos eventos de cada workflow conforme recebida na sequência de entrada, sem reparo implícito de história corrompida;
  - reidratação canônica e fail-closed: delegação integral da semântica de lifecycle e validação cronológica/transicional para `rehydrate_workflow(...)`, propagando `ValueError` em caso de anomalias (ex.: conclusão antes de abertura, duplicidades de eventos);
  - filtragem precisa de pendências: retenção exclusiva de workflows com `status == WorkflowStatus.PENDING_HUMAN_REVIEW` (`review is None`);
  - ordenação canônica determinística: ordenação estável por `(opened_at ASC, workflow_id ASC)` (FIFO com tie-break lexicográfico);
...
- **Incremento integrado da Issue #91 (Record Human Review Claim Application Use Case v1):**
  - módulo `src/agent_lab/human_review_claim_use_case.py`: introdução do boundary explícito da Camada de Aplicação para gravação de claims (`RecordHumanReviewClaimUseCase`);
  - injeção explícita do protocolo `HumanReviewClaimRepository` no construtor e método `execute(workflow, *, claim_id, specialist, claimed_at) -> HumanReviewClaim`;
  - coordenação estrita em duas fases: validação estrutural defensiva (`isinstance(workflow, GovernanceWorkflow) -> TypeError`) e delegação ao domínio via `claim_pending_human_review(...)` (Fase 1 / Zero I/O), seguida de persistência via `self._claim_repository.append(claim)` (Fase 2);
  - retorno direto da instância imutável de `HumanReviewClaim` persistida, sem DTOs redundantes;
  - propagação estritamente *fail-closed* de erros de domínio e de persistência (`HumanReviewClaimPersistenceError`, etc.), com exatamente uma tentativa de append, zero retries, zero rollbacks e zero mascaramento de exceções;
  - preservação integral do estado do workflow: permanece em `WorkflowStatus.PENDING_HUMAN_REVIEW` com `review is None`; zero emissão de `AuditEvent` e zero `WorkflowLifecycleEvent`;
  - teste de integração vertical pós-restart (`tests/test_human_review_claim_use_case_integration.py`) comprovando coordenação completa com `JsonlHumanReviewClaimRepository` real sobre arquivo temporário JSONL em disco e recuperação consistente via `get_by_id`, `list_by_workflow_id` e `list_all`;
  - exportação pública de `RecordHumanReviewClaimUseCase` no pacote raiz `src/agent_lab/__init__.py`;
  - fronteiras e escopo negativo estritamente preservados: repositório rejeita repetição de `claim_id` e permite múltiplos claims para o mesmo `workflow_id`; active claim, vigência, exclusividade, desempate, *Last-Claim-Wins*, locking/lease, ownership/assignment e `Queue with Claim Status` permanecem formalmente indefinidos e fora de escopo;
- **Incremento integrado da Issue #94 (Human Review Claim State Projection v1):**
  - módulo `src/agent_lab/human_review_claim_projection.py`: introdução da primeira projeção pura e determinística sobre fatos persistidos de `HumanReviewClaim`;
  - enum canônico `HumanReviewClaimFactState` com os estados factuais `NO_CLAIM`, `SINGLE_CLAIM` e `MULTIPLE_CLAIMS`;
  - read-model imutável `HumanReviewClaimState` (`frozen=True, slots=True`) com armazenamento estritamente limitado aos fatos fundamentais (`workflow_id: str` e `claims: tuple[HumanReviewClaim, ...]`);
  - eliminação total de estado derivado redundante: `state`, `claim_count`, `is_unclaimed`, `has_claims`, `has_multiple_claims` e `sole_claim` são implementados como propriedades puras (`@property`) computadas em tempo real a partir de `self.claims`, impossibilitando a construção de read-models internamente contraditórios;
  - função pura `project_human_review_claim_state(workflow_id: str, claims: Sequence[HumanReviewClaim]) -> HumanReviewClaimState` operando exclusivamente em memória com zero I/O;
  - validações defensivas nominais *fail-closed*: rejeição estrita de `workflow_id` não-string/booleano (`TypeError`) e vazio/whitespace (`ValueError`), rejeição de `claims` não-sequência ou sequências textuais/binárias (`str`, `bytes`, `bytearray` com `TypeError`);
  - validação prévia e exaustiva: todos os elementos da sequência de entrada são estritamente validados como `HumanReviewClaim` antes de qualquer filtragem por workflow (`TypeError`), impedindo que dados corrompidos ou inválidos sejam mascarados;
  - ordenação canônica determinística `(claimed_at ASC, claim_id ASC)` garantindo reprodutibilidade, auditabilidade e igualdade de read-models em permutações de entrada, sem conferir prioridade, autoridade operacional ou regra de *Last-Claim-Wins*;
  - semântica estrita de `sole_claim`: expõe o único fato existente quando `state == SINGLE_CLAIM` e retorna `None` quando `NO_CLAIM` ou `MULTIPLE_CLAIMS`, sem promessa de lock, titularidade ou exclusividade;
  - múltiplos claims expressam exclusivamente multiplicidade factual persistida no histórico, recusando desempates arbitrários;
  - sobrevivência e determinismo comprovados em teste de integração vertical pós-restart (`tests/test_human_review_claim_projection_integration.py`), provando na prática o princípio `Repository preserva ordem física | Projection interpreta ordem canônica`;
  - exportação pública de `HumanReviewClaimFactState`, `HumanReviewClaimState` e `project_human_review_claim_state` no pacote raiz `src/agent_lab/__init__.py`;
  - fronteiras e escopo negativo mantidos: zero eleição de active claim/winner/owner, zero vigência/TTL, zero exclusividade/lock/lease/SLA e zero mutação no ciclo de governança de `GovernanceWorkflow`.
- **Incremento integrado da Issue #97 (Pending Human Review Queue with Claim State Application Use Case v1):**
  - módulo `src/agent_lab/pending_human_reviews_with_claim_state_use_case.py`: introdução do boundary de composição da Camada de Aplicação (`ListPendingHumanReviewsWithClaimStateUseCase`);
  - read-model imutável `PendingHumanReviewWithClaimStateItem` (`frozen=True, slots=True`) compondo deterministicamente `workflow: GovernanceWorkflow` e `claim_state: HumanReviewClaimState`;
  - injeção explícita de dependências: o construtor recebe `WorkflowLifecycleRepository` e `HumanReviewClaimRepository`, preservando o desacoplamento da Application Layer em relação às implementações concretas de persistência;
  - método `execute() -> tuple[PendingHumanReviewWithClaimStateItem, ...]`;
  - composição somente-leitura da fila de pendências (`project_pending_human_review_queue`) com o estado factual de claims (`project_human_review_claim_state`);
  - pending queue estabelecida como conjunto condutor (*driver set*), retendo exclusivamente workflows pendentes;
  - preservação estrita da ordenação canônica FIFO `(opened_at ASC, workflow_id ASC)` da fila de pendências;
  - categorização factual de claims por item em `NO_CLAIM`, `SINGLE_CLAIM` e `MULTIPLE_CLAIMS`;
  - snapshot local único por repositório durante a execução: exatamente uma leitura de `workflow_lifecycle_repository.list_all_events()` e uma leitura global de `claim_repository.list_all()`, eliminando chamadas N+1 a `list_by_workflow_id()`;
  - validação relacional e fail-closed estrita: invariante `workflow.workflow_id == claim_state.workflow_id`, com propagação direta de exceções de integridade ou persistência sem mascaramento e sem retorno de resultados parciais;
  - semântica estritamente factual de `sole_claim`: expressa unicamente a cardinalidade factual persistida quando `state == SINGLE_CLAIM`, sem semântica operacional de active claim, owner, assignment, winner, exclusividade ou locking;
  - limites arquiteturais estritamente respeitados: zero active-claim policy, zero ownership operacional, zero assignment gerencial, zero locking/lease/SLA e zero mutação persistente (operação estritamente de leitura);
  - teste de integração vertical pós-restart (`tests/test_pending_human_reviews_with_claim_state_use_case_integration.py`) comprovando coordenação completa sobre repositórios JSONL reais (`JsonlWorkflowLifecycleRepository` e `JsonlHumanReviewClaimRepository`) após reinicialização física das instâncias;
  - símbolos contextuais no módulo da funcionalidade, sem exportação na API pública do pacote raiz `src/agent_lab/__init__.py`.
- **Incremento integrado da Issue #100 (Reviewer Claim Eligibility Policy v1):**
  ...
  - preservação da autoridade da projeção de claims: `MULTIPLE_CLAIMS` resulta obrigatoriamente em `MULTIPLE_CLAIMS_CONFLICT`, inclusive se múltiplos claims registrados pertencerem ao mesmo principal estável, recusando desempates arbitrários, merges de claims ou eleição de vencedor em conformidade com o princípio de separação de responsabilidades;
  - validação defensiva *fail-closed*: `TypeError` para `claim_state` que não seja `HumanReviewClaimState` e para `reviewer_identity` que não seja `VerifiedSpecialistIdentity`;
  - exportação pública canônica dos símbolos `ReviewerEligibilityStatus`, `ReviewerEligibilityDecision` e `evaluate_reviewer_claim_eligibility` no pacote raiz `src/agent_lab/__init__.py`;
  - pureza arquitetural estritamente preservada: a Policy apenas classifica elegibilidade normativa em memória, com zero I/O, sem garantias de identidade real ou autenticação real, sem ownership, sem exclusividade, sem mutação de entidades e sem enforcement em tempo de execução (`RecordHumanDecisionUseCase` permaneceu intacto sem restrição de chamador naquela etapa, antes da Issue #103).
- **Incremento integrado da Issue #103 (Reviewer Claim Eligibility Runtime Enforcement v1):**
  - escopo: aplicação do gate obrigatório pré-write de elegibilidade em runtime no caso de uso `RecordHumanDecisionUseCase`;
  - coordenação em quatro fases canônicas de execução (Fase 1: domínio em memória com zero I/O; Fase 2: gate pré-write de elegibilidade; Fase 3: persistência sequencial não-atômica `Audit → Lifecycle`; Fase 4: retorno imutável de `RecordHumanDecisionResult`);
  - injeção de dependência obrigatória de `HumanReviewClaimRepository` (`claim_repository`);
  - exatamente uma leitura física no repositório de claims (`claim_repository.list_by_workflow_id(workflow.workflow_id)`) após validação da Fase 1;
  - reutilização direta da Projeção de Claims da Issue #94 (`project_human_review_claim_state`) e da Política de Elegibilidade da Issue #100 (`evaluate_reviewer_claim_eligibility`);
  - fail-closed: decisões normativas `CLAIM_REQUIRED`, `CLAIMANT_MISMATCH` e `MULTIPLE_CLAIMS_CONFLICT` bloqueiam writes e levantam `ReviewerNotEligibleError` (`0 writes`);
  - conformidade de identidade: especialista com Stable Principal coincidente `(specialist_id, identity_provider, identity_subject)` preserva elegibilidade com novos metadados de verificação (`verification_id`, `verified_at`);
  - falha física ou corrupção do repositório de claims propaga a exceção original de forma fail-closed sem conversão para erro de negócio (`0 writes`);
  - validação vertical em teste de integração (`tests/test_human_review_use_case_integration.py`) comprovando ciclo completo com `JsonlHumanReviewClaimRepository` e integração pós-restart com nova instância sobre o mesmo JSONL;
  - preservação estrita dos limites arquiteturais: zero active claim, zero owner, zero winner, zero assignment, zero locking/lease/TTL e zero SLA;
...
- **Incremento integrado da Issue #106 (Human Review Claim Release Domain Contract v1):**
  ...
  - comprovação rigorosa de imutabilidade: `workflow` e `claim` originais permanecem 100% inalterados em memória, com zero efeitos colaterais;
  - exportação pública formal de `HumanReviewClaimRelease` e `release_human_review_claim` no pacote raiz `src/agent_lab/__init__.py` e inclusão em `__all__`;
  - limites arquiteturais estritamente preservados: zero persistência durável de releases, zero serialização/versionamento de releases, zero Application Use Case de release, zero Active Claim Projection, zero Active Claim Policy, zero ownership, zero assignment, zero winner, zero exclusividade, zero lease/TTL/SLA, zero mutação de workflow e zero eventos de ciclo de vida ou auditoria;
  - baseline integrado elevado de 579 para 595 testes aprovados (100% GREEN).

> [!NOTE]
> **Weekly Architectural Review #01 (2026-09-04) — Aprendizado de Governança:**
> A Issue #97 foi o primeiro incremento funcional derivado diretamente da Weekly Architectural Review #01 (2026-09-04). O ciclo de engenharia seguiu rigorosamente a esteira de governança:
> Weekly Architectural Review → hipótese depurada → Issue #97 → SPEC 0097 → micro-TDD → Pre-PR Gate → Evidence Reconciliation → PR #98 → merge.
> O processo consolidou a máxima: *“Testes provam comportamento; gates provam prontidão.”*
> Em particular, o Pre-PR Gate detectou trailing whitespace documental antes da publicação da branch, demonstrando na prática que prontidão arquitetural engloba conformidade e integridade da evidência, não apenas suíte GREEN.

> [!NOTE]
> **Issue #100 (2026-09-05) — Aprendizados de Governança e Engenharia:**
> O ciclo da Issue #100 consolidou importantes lições arquiteturais e de processo:
> - **Functional completion ≠ Issue completion:** a conclusão de fatias de código e o merge do PR funcional não encerram a Issue; o encerramento canônico de uma Issue de governança requer closeout documental rigoroso e alinhamento do ecossistema.
...
- **Incremento integrado da Issue #124 (Ground Truth Decision Recommendation Evaluator v1):**
  - módulo `src/agent_lab/ground_truth_evaluation.py`: introdução da camada pura de domínio e metrologia em memória (zero-I/O) para aferição de recomendações algorítmicas de governança (`DecisionRecommendation`) contra referências formais de Ground Truth (`DecisionRecommendationGroundTruth`);
  - dataclass imutável `DecisionRecommendationCaseEvaluation` (`frozen=True, slots=True` sem `__dict__`) representando o diagnóstico atômico de caso individual com `evaluation_case_id`, `ground_truth_id`, `material_id`, `expected_decision: GovernanceDecision`, `predicted_decision: GovernanceDecision`, e propriedades booleanas derivadas `is_match` e `is_mismatch`;
  - dataclass imutável `DecisionRecommendationEvaluationReport` (`frozen=True, slots=True` sem `__dict__`) com `dataset_id: str` obrigatório, ordenação canônica compulsória por tupla composta `(evaluation_case_id, ground_truth_id)` e métricas derivadas (`total_cases`, `matched_cases`, `mismatched_cases`, `accuracy: float | None` com `accuracy is None` para coleção vazia, `is_empty`, `is_perfect_match`, `matches`, `mismatches`);
  - função pura atômica `evaluate_decision_recommendation(ground_truth, prediction) -> DecisionRecommendationCaseEvaluation` com validação de tipos nominais (`TypeError`) e fail-closed por divergência de `material_id` (`ValueError`);
  - função pura de lote `evaluate_decision_recommendations(dataset, predictions) -> DecisionRecommendationEvaluationReport` aceitando estritamente `Mapping[str, DecisionRecommendation]` indexado por `evaluation_case_id`, exigindo paridade exata 1:1 de chaves (rejeitando missing/extra keys com `ValueError`), validando `material_id` para cada par via delegação à função atômica e garantindo independência da ordem de inserção;
  - distinção entre identidade experimental (`evaluation_case_id`) e identidade da entidade de domínio (`material_id`), preservando pareamento metrológico inequívoco;
  - limites estritos preservados: zero I/O, zero persistência própria, exatidão categórica (accuracy/exact-match) sem métricas estatísticas binárias/multiclasses (Precision, Recall, F1), sem matrizes de confusão e sem loaders ou runners externos;
  - exportação pública canônica dos quatro símbolos no pacote raiz `src/agent_lab/__init__.py` e inclusão em `__all__`;
  - baseline integrado elevado de 776 para 830 testes aprovados (100% GREEN) com a inclusão de 54 testes unitários e defensivos em `tests/test_ground_truth_evaluation.py`.
- **Incremento integrado da Issue #129 (Ground Truth Material Rule Evaluator v1):**
  - módulo `src/agent_lab/ground_truth_evaluation.py`: introdução da camada pura de domínio e metrologia em memória (zero-I/O) para aferição de predições multirrótulo de regras cadastrais de materiais (`MaterialRulePrediction`) contra referências formais de Ground Truth (`MaterialRuleGroundTruth`);
  - dataclass imutável `MaterialRulePrediction` (`frozen=True, slots=True`) com `material_id: str` e `predicted_issue_types: tuple[IssueType, ...]`, bloqueio fail-closed de `IssueType.POSSIBLE_DUPLICATE`, rejeição de duplicatas sem deduplicação silenciosa e ordenação determinística interna;
  - dataclass imutável `MaterialRuleCaseEvaluation` (`frozen=True, slots=True`) diagnosticando `true_positives`, `false_positives`, `false_negatives` (como tuplas imutáveis ordenadas), `is_clean_match`, `is_defect_match`, `is_match`, `is_mismatch`, `has_false_positives`, `has_false_negatives`;
  - dataclass imutável `MaterialRuleEvaluationReport` (`frozen=True, slots=True`) com `dataset_id: str` obrigatório, ordenação canônica compulsória por `(evaluation_case_id, ground_truth_id)`, métricas agregadas (`total_cases`, `matched_cases`, `mismatched_cases`, `accuracy` / `exact_match_ratio`, `total_true_positives`, `total_false_positives`, `total_false_negatives`) e semântica de dataset vazio (`accuracy is None`);
  - funções puras `evaluate_material_rule` e `evaluate_material_rules` com pareamento determinístico por `evaluation_case_id`, validação relacional por `material_id` e exatidão categórica;
  - exportação pública canônica dos cinco símbolos no pacote raiz `src/agent_lab/__init__.py` e inclusão em `__all__`;
  - baseline integrado elevado de 830 para 883 testes aprovados (100% GREEN) com a inclusão de 53 testes unitários e defensivos em `tests/test_ground_truth_evaluation.py`.
- **Incremento integrado da Issue #133 (Ground Truth Duplicate Pair Evaluator v1):**
  - módulo `src/agent_lab/ground_truth_evaluation.py`: introdução da camada pura de metrologia e avaliação em memória (zero-I/O) para aferição de predições de duplicidade cadastral de pares de materiais (`DuplicatePairPrediction`) contra referências formais de Ground Truth (`DuplicatePairGroundTruth`);
  - dataclass imutável `DuplicatePairPrediction` (`frozen=True, slots=True`) com `material_id_a: str`, `material_id_b: str` e `is_duplicate: bool`, com validação fail-closed impedindo auto-pares (`material_id_a == material_id_b`) e pares invertidos (`material_id_a > material_id_b`) sem inversão silenciosa;
  - dataclass imutável `DuplicatePairCaseEvaluation` (`frozen=True, slots=True`) diagnosticando a correspondência com `is_match` e `is_mismatch` e preservando a linhagem do caso (`evaluation_case_id`, `ground_truth_id`, `material_id_a`, `material_id_b`, `expected_is_duplicate`, `predicted_is_duplicate`);
  - dataclass imutável `DuplicatePairEvaluationReport` (`frozen=True, slots=True`) com `dataset_id: str` obrigatório, ordenação canônica determinística compulsória por `(evaluation_case_id, ground_truth_id)`, métricas puras de exact-match (`total_cases`, `matched_cases`, `mismatched_cases`, `accuracy: float | None`), semântica formal de dataset vazio (`accuracy is None`, `is_empty is True`), e propriedades derivadas `is_perfect_match`, `matches`, `mismatches`;
  - função pura atômica `evaluate_duplicate_pair(ground_truth, prediction) -> DuplicatePairCaseEvaluation` com validação relacional fail-closed de `material_id_a` e `material_id_b`;
  - função pura de lote `evaluate_duplicate_pairs(dataset, predictions: Mapping[str, DuplicatePairPrediction]) -> DuplicatePairEvaluationReport` indexada estritamente por `evaluation_case_id` com paridade exata 1:1 (rejeitando chaves ausentes e excedentes com `ValueError`), validação relacional e independência de ordem de inserção;
  - exportação pública canônica dos cinco símbolos (`DuplicatePairPrediction`, `DuplicatePairCaseEvaluation`, `DuplicatePairEvaluationReport`, `evaluate_duplicate_pair`, `evaluate_duplicate_pairs`) no pacote raiz `src/agent_lab/__init__.py` e inclusão em `__all__`;
  - baseline integrado elevado de 883 para 936 testes aprovados (100% GREEN) com a inclusão de 53 testes unitários e defensivos em `tests/test_ground_truth_evaluation.py`.
- **Incremento integrado da Issue #137 (Ground Truth Atomic Serialization v1):**
  - módulo `src/agent_lab/ground_truth_serialization.py`: introdução da camada pura de serialização versionada em memória (zero-I/O, determinística, fail-closed, closed-schema) para os três contratos atômicos de Ground Truth (`MaterialRuleGroundTruth`, `DuplicatePairGroundTruth`, `DecisionRecommendationGroundTruth`);
  - envelope versionado canônico com `schema_version = 1` explícito e compulsório, e discriminadores canônicos de tipo (`RECORD_TYPE_MATERIAL_RULE_GROUND_TRUTH`, `RECORD_TYPE_DUPLICATE_PAIR_GROUND_TRUTH`, `RECORD_TYPE_DECISION_RECOMMENDATION_GROUND_TRUTH`);
  - funções dedicadas e específicas de serialização e desserialização: `material_rule_ground_truth_to_record` / `material_rule_ground_truth_from_record`, `duplicate_pair_ground_truth_to_record` / `duplicate_pair_ground_truth_from_record`, `decision_recommendation_ground_truth_to_record` / `decision_recommendation_ground_truth_from_record`;
  - garantias fail-closed e closed-schema: rejeição de campos ausentes, extras ou desconhecidos, validação estrita de canonicalidade de strings (`val == val.strip()`) sem reparo silencioso (`.strip()`), validação estrita de listas de `expected_issue_types` na ordem canônica por `IssueType.value` sem reordenação silenciosa, validação relacional de pares de duplicidade (`material_id_a < material_id_b`), validação de enum de decisão (`GovernanceDecision`) serializado como string primitiva canônica (com rejeição estrita de instâncias de `StrEnum` no payload serializado), e timestamps timezone-aware em formato ISO 8601 preservando instante e offset UTC;
  - preservação integral de proveniência (`LabelProvenance.SPECIALIST_CURATED` com sub-schema estrito de `VerifiedSpecialistIdentity` e invariante relacional temporal `annotator.verified_at <= labeled_at`, ou `SYNTHETIC_SPECIFIED` com `annotator: None`);
  - governança de exportação: exportação canônica dos 9 símbolos públicos não-ambíguos (3 discriminadores `RECORD_TYPE_*` e 6 funções específicas de conversão) no pacote raiz `src/agent_lab/__init__.py` e inclusão em `__all__`, mantendo `SCHEMA_VERSION_V1` deliberadamente module-scoped em `agent_lab.ground_truth_serialization` para evitar colisão semântica no pacote raiz;
  - baseline integrado elevado de 936 para 1015 testes aprovados (100% GREEN) com a inclusão de 79 testes unitários e defensivos em `tests/test_ground_truth_serialization.py`.
- **Incremento integrado da Issue #141 (Ground Truth Decision Recommendation Benchmark Use Case v1):**
  - módulo `src/agent_lab/decision_recommendation_benchmark_use_case.py`: introdução do primeiro boundary da camada de aplicação para orquestração ponta a ponta de benchmark de recomendações de governança contra Ground Truth em memória (zero-I/O);
  - dataclass imutável `DecisionRecommendationBenchmarkCase` (`frozen=True, slots=True`) associando explicitamente `evaluation_case_id: str` a `material: MaterialRecord`, formalizando a independência semântica entre identidade experimental e cadastral (permitindo coincidência textual explícita sem inferência ou derivação implícita entre elas) e validação fail-closed;
  - caso de uso `RunDecisionRecommendationBenchmarkUseCase` coordenando a execução: validação estrutural de entrada (`dataset` e `cases`), rejeição fail-closed de duplicatas de `evaluation_case_id`, invocação do pipeline de recomendação determinístico, validação relacional fail-closed `prediction.material_id == case.material.material_id`, montagem indexada de `predictions[evaluation_case_id]`, delegação da metrologia a `evaluate_decision_recommendations(dataset, predictions)` e retorno do relatório imutável `DecisionRecommendationEvaluationReport`;
  - pipeline determinístico default real integrado (`DeterministicGovernanceValidator().analyze(material) → evidence_collection → recommend_decision`) com validação fail-closed contra evidências ausentes, e suporte a `pipeline` injetável (`RecommendationPipeline`) para experimentação desacoplada;
  - preservação do princípio arquitetural *"Application coordena; Pipeline produz; Evaluator mede"*, sem recálculo de acurácia no use case e sem persistência em disco;
  - exportação pública canônica dos dois símbolos (`DecisionRecommendationBenchmarkCase` e `RunDecisionRecommendationBenchmarkUseCase`) no pacote raiz `src/agent_lab/__init__.py` e inclusão em `__all__`;
  - baseline integrado elevado de 1015 para 1045 testes aprovados (100% GREEN) com a inclusão de 30 testes unitários, defensivos e de integração em `tests/test_decision_recommendation_benchmark_use_case.py`.
...
- oito boundaries de Application integrados:
  ...
  4. `ListPendingHumanReviewsWithClaimStateUseCase` (permanece 100% suportado e testado)
...
- permanecem estritamente fora do escopo (não implementados):
  ...
  - eleição de vencedor (*winner*);
  - exclusividade;
  - desempate / *First-Claim-Wins* / *Last-Claim-Wins*;
  - garantia de unicidade concorrente/global de claim;
  - *locking* / *checkout*;
  - liberação forçada (*force-release*) ou transferência / *reassignment* de titularidade;
  - vigência / TTL / *lease* / expiração / SLA;
  - priorização operacional de fila;
  - interface operacional (UI/Streamlit / `app.py`), APIs REST, CLI;
  - processamento assíncrono / concorrência multiprocesso e otimizações P-07;
  - ausência de locking multiprocesso, autenticação/autorização real (RBAC) ou integração com ERP;
...
Contrato
  ...
  → Record Human Review Claim Application Use Case (concluído na #91)
  → Human Review Claim State Projection (concluída na #94)
  → Pending Human Review Queue with Claim State Application Use Case (concluído na #97)
  → Reviewer Claim Eligibility Policy v1 (concluída na #100)
  ...
  → Ground Truth Decision Recommendation Evaluator v1 (concluída na #124 via PR #126 / merge 05eed90)
  → Ground Truth Material Rule Evaluator v1 (concluída na #129 via PR #131 / merge 68c99bd)
  ...
  → Catalog CSV Ingestion Adapter v1 (concluída na #154 via PR #156 / merge 2b8bc94)
...
### 6.13 Avaliação de Ground Truth (Contratos de Domínio e Avaliadores)

```text
src/agent_lab/ground_truth.py
src/agent_lab/ground_truth_evaluation.py
```
...
Fluxo conceitual de avaliação:

```text
Ground Truth Dataset
        +
Predictions Mapping (indexado por evaluation_case_id)
        ↓
pareamento determinístico por evaluation_case_id (paridade 1:1)
        ↓
validação relacional fail-closed por material_id / (material_id_a, material_id_b)
        ↓
CaseEvaluation (DecisionRecommendationCaseEvaluation / MaterialRuleCaseEvaluation / DuplicatePairCaseEvaluation)
        ↓
EvaluationReport (DecisionRecommendationEvaluationReport / MaterialRuleEvaluationReport / DuplicatePairEvaluationReport)
        ↓
métricas determinísticas puras (accuracy; exact_match_ratio adicionalmente no Material Rule evaluator)
```
...
- avaliar atômica e deterministicamente predições contra gabaritos:
  - **Decision Recommendation evaluator:** avalia `DecisionRecommendation` contra `DecisionRecommendationGroundTruth` (`evaluate_decision_recommendation` → `DecisionRecommendationCaseEvaluation`) e em lote (`evaluate_decision_recommendations` → `DecisionRecommendationEvaluationReport`), consumindo estritamente `Mapping[str, DecisionRecommendation]` indexado por `evaluation_case_id` com exact-match categórico de decisão escalar;
  - **Material Rule evaluator:** avalia predições de regras (`MaterialRulePrediction`) contra `MaterialRuleGroundTruth` (`evaluate_material_rule` → `MaterialRuleCaseEvaluation`) e em lote (`evaluate_material_rules` → `MaterialRuleEvaluationReport`), consumindo estritamente `Mapping[str, MaterialRulePrediction]` indexado por `evaluation_case_id` com exact set match multirrótulo sobre `IssueType`, diagnóstico determinístico de conjuntos (`true_positives`, `false_positives`, `false_negatives`), distinção semântica de correspondência (`is_clean_match` e `is_defect_match`), flags de anomalia (`has_false_positives`, `has_false_negatives`), bloqueio fail-closed de `IssueType.POSSIBLE_DUPLICATE` e rejeição fail-closed de duplicatas em `predicted_issue_types` (sem deduplicação silenciosa);
  - **Duplicate Pair evaluator:** avalia predições de pares (`DuplicatePairPrediction`) contra `DuplicatePairGroundTruth` (`evaluate_duplicate_pair` → `DuplicatePairCaseEvaluation`) e em lote (`evaluate_duplicate_pairs` → `DuplicatePairEvaluationReport`), consumindo estritamente `Mapping[str, DuplicatePairPrediction]` indexado por `evaluation_case_id` com exact-match categórico de duplicidade binária (`is_duplicate: bool`), validação relacional fail-closed de `material_id_a` e `material_id_b`, e bloqueio fail-closed de auto-pares (`material_id_a == material_id_b`) e pares invertidos (`material_id_a > material_id_b`);
- garantir que as APIs de avaliação em lote consumam estritamente mapeamentos deterministicamente indexados por `evaluation_case_id`:
  - `Mapping[str, DecisionRecommendation]` no evaluator de recommendation;
  - `Mapping[str, MaterialRulePrediction]` no evaluator de material rules;
  - `Mapping[str, DuplicatePairPrediction]` no evaluator de duplicate pairs;
  todos com paridade exata 1:1 de chaves e validação relacional de identificadores de materiais;
- garantir imutabilidade estrita (`frozen=True, slots=True`), ordenação canônica determinística de casos por `(evaluation_case_id, ground_truth_id)`, semântica explícita de coleção vazia (`accuracy is None` em todos os reports; `exact_match_ratio is None` adicionalmente no Material Rule evaluator), cálculo de exatidão sem arredondamento interno e validações defensivas fail-closed;
- sustentar baseline consolidado de **936 testes aprovados** (100% GREEN) com exportações públicas no pacote raiz `src/agent_lab/__init__.py`;
- preservar separações de autoridade: `Ground Truth ≠ Prediction`, `Dataset ≠ Metric`, `Evaluation Contract ≠ Benchmark Result`, `evaluation_case_id` é identidade experimental e não deve ser confundido com a identidade da entidade avaliada (`material_id` ou o par `(material_id_a, material_id_b)`);
...
- manter fora de escopo nesta etapa: Precision, Recall, F1 multiclasse, matriz de confusão, calibração de thresholds, benchmark runner, persistência em disco/banco e loaders externos.
...
- `RecordHumanReviewClaimUseCase`: caso de uso de aplicação responsável por coordenar a assunção voluntária de um workflow pendente por especialista e sua persistência append-only;
- `PendingHumanReviewWithClaimStateItem`: read-model imutável associando deterministicamente um workflow pendente ao seu estado factual de claims;
- `ListPendingHumanReviewsWithClaimStateUseCase`: caso de uso de aplicação responsável por consultar a fila pendente combinada de forma somente-leitura com o estado factual histórico de claims (`NO_CLAIM`, `SINGLE_CLAIM`, `MULTIPLE_CLAIMS`);
...
- em `RecordHumanDecisionUseCase`:
  - coordenar a decisão humana através de quatro fases canônicas de execução com injeção obrigatória de `HumanReviewClaimRepository` (`claim_repository`) e `HumanReviewClaimReleaseRepository` (`claim_release_repository`):
    - **Fase 1 — domínio em memória (zero-I/O):** validação estrutural dos argumentos de fronteira (`isinstance(workflow, GovernanceWorkflow)`), execução de `record_human_review(...)`, derivação do workflow concluído via `conclude_governance_workflow(...)` e construção do evento `WorkflowConcluded(...)`; qualquer falha de domínio interrompe imediatamente antes de qualquer leitura de claims/releases ou escrita (`0 claim/release reads → 0 writes`);
    - **Fase 2 — gate pré-write release-aware:** executada somente após Fase 1 válida, realiza targeted reads nos repositórios na ordem causal estrita (`claims read → releases read → Projection → Policy → gate`):
      1. lê os claims do workflow via `claims = claim_repository.list_by_workflow_id(workflow.workflow_id)` (falha física ou corrupção interrompe fail-closed antes da leitura de releases com `0 writes`);
      2. lê os releases do workflow via `releases = claim_release_repository.list_by_workflow_id(workflow.workflow_id)` (falha física ou corrupção interrompe fail-closed antes de qualquer escrita com `0 writes`);
      3. projeta o estado factual release-aware via `project_release_aware_claim_state(workflow.workflow_id, claims, releases)`;
      4. avalia a autoridade normativa via política pura `evaluate_release_aware_reviewer_claim_eligibility(release_aware_state, reviewer_identity)`;
      5. se a decisão for `ELIGIBLE`: autoriza o prosseguimento para a persistência;
      6. se a decisão for diferente de `ELIGIBLE` (`CLAIM_REQUIRED`, `CLAIMANT_MISMATCH`, `MULTIPLE_CLAIMS_CONFLICT`): interrompe imediatamente levantando `ReviewerNotEligibleError` com a decisão normativa, garantindo zero escritas (`0 writes`);
      7. nenhuma exceção de I/O é mascarada;
    - **Fase 3 — persistência:** dual-write sequencial e não-atômico em `AuditRepository` (`audit_repository.append(human_review_result.audit_event)`) e `WorkflowLifecycleRepository` (`workflow_lifecycle_repository.append_concluded(lifecycle_event)`); falhas de persistência propagam exceções fail-closed sem rollback artificial;
    - **Fase 4 — retorno:** retorna a estrutura imutável `RecordHumanDecisionResult(workflow, review, audit_event, lifecycle_event)` preservando integralmente o contrato de retorno inalterado;
  - preservar estritamente os limites arquiteturais: zero active claim, zero owner, zero winner, zero assignment e zero locking;
- em `ListPendingHumanReviewsUseCase`:
  ...
  - operação somente-leitura, sem estado mutável, sem cache e sem I/O direto fora do repositório;
- em `RecordHumanReviewClaimUseCase`:
  - validar estruturalmente o workflow de entrada (`isinstance(workflow, GovernanceWorkflow)`);
  - delegar a criação do claim ao domínio (`claim_pending_human_review`) garantindo validação em memória com zero I/O;
  - disparar a persistência append-only via `HumanReviewClaimRepository.append(claim)`;
  - propagar falhas de domínio e persistência de forma *fail-closed*;
- em `ListPendingHumanReviewsWithClaimStateUseCase`:
  - obter fatos históricos através de snapshot local único por repositório (`workflow_lifecycle_repository.list_all_events()` e `claim_repository.list_all()`), eliminando N+1;
  - compor deterministicamente a fila pendente com o estado factual de claims via `project_pending_human_review_queue` e `project_human_review_claim_state`;
  - retornar `tuple[PendingHumanReviewWithClaimStateItem, ...]`, preservando a pending queue como driver set FIFO;
...
- em `ListPendingHumanReviewsWithReleaseAwareClaimStateUseCase`:
  - obter snapshot global único de lifecycle, claims e releases (`workflow_lifecycle_repository.list_all_events()`, `claim_repository.list_all()` e `claim_release_repository.list_all()`), eliminando N+1 porque cada repositório é lido uma única vez;
  - projetar a pending queue a partir do snapshot de lifecycle via `project_pending_human_review_queue(events_snapshot)`;
  - para cada workflow pendente, chamar `project_release_aware_claim_state(workflow.workflow_id, claims_snapshot, releases_snapshot)`, deixando a filtragem por `workflow_id` sob responsabilidade estrita da Projection;
...
- Incremento da Issue #88: +41 testes sobre o baseline de entrada de 462 (18 testes em `tests/test_human_review_claim_serialization.py` + 18 testes em `tests/test_human_review_claim_repository.py` + 5 testes de integração em `tests/test_human_review_claim_persistence_integration.py`)
- Baseline integrado após a Issue #88: 503 testes
- Incremento da Issue #91: +6 testes sobre o baseline de entrada de 503 (4 testes unitários em `tests/test_human_review_claim_use_case.py` + 2 testes de integração em `tests/test_human_review_claim_use_case_integration.py`)
- Baseline integrado após a Issue #91: 509 testes
- Incremento da Issue #94: +22 testes sobre o baseline de entrada de 509 (19 testes unitários/defensivos em `tests/test_human_review_claim_projection.py` + 3 testes de integração em `tests/test_human_review_claim_projection_integration.py`)
- Baseline integrado após a Issue #94: 531 testes
- Incremento da Issue #97: +18 testes sobre o baseline de entrada de 531 (17 testes unitários em `tests/test_pending_human_reviews_with_claim_state_use_case.py` + 1 teste de integração em `tests/test_pending_human_reviews_with_claim_state_use_case_integration.py`)
- Baseline integrado após a Issue #97: 549 testes
- Incremento da Issue #100: +20 testes sobre o baseline de entrada de 549 (20 testes unitários em `tests/test_reviewer_eligibility_policy.py`)
- Baseline integrado após a Issue #100: 569 testes
- Incremento da Issue #103: +10 testes sobre o baseline de entrada de 569 (7 testes unitários em `tests/test_human_review_use_case.py` + 3 testes de integração em `tests/test_human_review_use_case_integration.py`)
- Baseline integrado após a Issue #103: 579 testes
- Incremento da Issue #106: +16 testes sobre o baseline de entrada de 579 (16 testes unitários em `tests/test_human_review_claim.py`: 7 em `HumanReviewClaimReleaseTests` e 9 em `ReleaseHumanReviewClaimFunctionTests`, além de expansão do teste de exportação pública)
...
- Incremento da Issue #124: +54 testes sobre o baseline de entrada de 776 (54 testes unitários e defensivos em `tests/test_ground_truth_evaluation.py` cobrindo `DecisionRecommendationCaseEvaluation`, `DecisionRecommendationEvaluationReport`, `evaluate_decision_recommendation`, `evaluate_decision_recommendations` e exports públicos no package root).
- Baseline integrado após a Issue #124: 830 testes (100% GREEN)
- Incremento da Issue #129: +53 testes sobre o baseline de entrada de 830 (53 testes unitários e defensivos em `tests/test_ground_truth_evaluation.py` cobrindo `MaterialRulePrediction`, `MaterialRuleCaseEvaluation`, `MaterialRuleEvaluationReport`, `evaluate_material_rule`, `evaluate_material_rules` e exports públicos no package root).
- Baseline integrado após a Issue #129: 883 testes (100% GREEN)
- Incremento da Issue #133: +53 testes sobre o baseline de entrada de 883 (53 testes unitários e defensivos em `tests/test_ground_truth_evaluation.py` cobrindo `DuplicatePairPrediction`, `DuplicatePairCaseEvaluation`, `DuplicatePairEvaluationReport`, `evaluate_duplicate_pair`, `evaluate_duplicate_pairs` e exports públicos no package root).
- Baseline integrado após a Issue #133: 936 testes (100% GREEN)
- Incremento da Issue #137: +79 testes sobre o baseline de entrada de 936 (79 testes unitários e defensivos em `tests/test_ground_truth_serialization.py` cobrindo round-trip semanticamente lossless, validações fail-closed e closed-schema para os três contratos atômicos, regras temporais, canonicalidade estrita e exports públicos no package root com `SCHEMA_VERSION_V1` mantido module-scoped).
- Baseline integrado após a Issue #137: 1015 testes (100% GREEN)
- Incremento da Issue #141: +30 testes sobre o baseline de entrada de 1015 (30 testes unitários, defensivos e de integração em `tests/test_decision_recommendation_benchmark_use_case.py` cobrindo o contrato `DecisionRecommendationBenchmarkCase`, independência semântica de identidades, execução nominal com pipeline default, pipeline injetável, integridade relacional `prediction.material_id == case.material.material_id`, rejeição de duplicatas de `evaluation_case_id`, delegação fail-closed para o evaluator oficial com detecção de faltantes/excedentes/mismatches, dataset vazio e exports públicos canônicos no package root).
- Baseline integrado após a Issue #141: 1045 testes (100% GREEN)
- Incremento da Issue #145: +57 testes sobre o baseline de entrada de 1045, distribuídos em:
  - `tests/test_human_review_claim_projection.py`: +22
  - `tests/test_reviewer_eligibility_policy.py`: +11
  - `tests/test_pending_human_reviews_with_claim_state_use_case.py`: +18
  - `tests/test_human_review_use_case.py`: +4
  - `tests/test_human_review_use_case_integration.py`: +1
...
- Incremento da Issue #154: +21 testes sobre o baseline de entrada de 1173, distribuídos em:
  - `tests/test_catalog_csv_adapter.py`: +21 (Slice 1: 8 testes; Slice 2: 10 testes; Slice 3: 3 testes)
- Baseline integrado após a Issue #154: 1194 testes (100% GREEN)
...
- persistência de ground truth e datasets em disco/banco, loaders externos (JSONL/CSV/Parquet), computação de métricas (Precision, Recall, F1), benchmark runners para regras de materiais e duplicidades, carga de datasets externos, calibração de thresholds e consenso/adjudicação continuam adiadas (os contratos atômicos de domínio puro de Ground Truth v1 foram integrados na Issue #115, os contratos de Ground Truth Datasets v1 foram integrados na Issue #119, o avaliador de recomendação de governança v1 foi integrado na Issue #124, o avaliador de regras cadastrais de materiais v1 foi integrado na Issue #129, o avaliador de pares duplicados v1 foi integrado na Issue #133, a serialização atômica versionada pura em memória foi integrada na Issue #137 e o application use case de benchmark para recomendações de decisão foi integrado na Issue #141);
...
- PoC vendável de diagnóstico de qualidade cadastral — núcleo computacional puro de domínio e aplicação em memória integrado na Issue #149 e adaptador operacional de ingestão física de catálogo CSV integrado na Issue #154; interfaces (UI/Streamlit, CLI, API REST), outros formatos de arquivos (Excel, Parquet, JSONL operacional), persistência de relatórios em disco e integrações externas (ERP) permanecem etapas futuras;
...
MAIN INTEGRADA:
- Baseline integrado na main: 1194 testes | unittest | Python 3.11.
- Última entrega funcional integrada na main: Issue #154 | Catalog CSV Ingestion Adapter v1 | PR #156 (merge 2b8bc94).
- Última SPEC integrada: docs/specs/0154_catalog_csv_ingestion_adapter_v1.md (Status: IMPLEMENTED).
- Último PR funcional integrado: PR #156.
- Último merge funcional: 2b8bc94.
- Arquitetura integrada: Regras + LLM estruturada + evidências + recomendação + identidade verificável
  ...
  + camada de aplicação com RecordHumanDecisionUseCase executando coordenação em quatro fases canônicas (Fase 1: domínio em memória com zero I/O; Fase 2: gate pré-write de elegibilidade via claim_repository, projeção factual e política pura; Fase 3: persistência sequencial não-atômica audit/lifecycle; Fase 4: retorno do resultado) com resultado imutável RecordHumanDecisionResult
  ...
  + persistência append-only durável em JSONL de HumanReviewClaim via JsonlHumanReviewClaimRepository com schema_version=1 canônico, validação fail-closed e integridade pós-restart
  + boundary de gravação na camada de aplicação via RecordHumanReviewClaimUseCase coordenando validação de domínio em memória (zero I/O) e persistência append-only de claims
  + projeção pura determinística de estado de claims via project_human_review_claim_state classificando fielmente NO_CLAIM, SINGLE_CLAIM e MULTIPLE_CLAIMS em read-model imutável HumanReviewClaimState com propriedades puramente derivadas e ordenação canônica por (claimed_at ASC, claim_id ASC)
  + boundary de aplicação para consulta da fila pendente com estado factual de claims via ListPendingHumanReviewsWithClaimStateUseCase compondo deterministicamente project_pending_human_review_queue e project_human_review_claim_state no read-model PendingHumanReviewWithClaimStateItem com snapshot local único por repositório, driver set FIFO, zero N+1 e propagação fail-closed
  ...
  + camada de avaliação pura de recomendações de governança contra ground truth em memória (DecisionRecommendationCaseEvaluation, DecisionRecommendationEvaluationReport, evaluate_decision_recommendation e evaluate_decision_recommendations) com pareamento estrito por evaluation_case_id, validação relacional de material_id, exact-match categórico (accuracy), linhagem compulsória por dataset_id, semântica de dataset vazio (accuracy=None), ordenação canônica determinística por (evaluation_case_id, ground_truth_id), imutabilidade (frozen/slots), zero I/O e exports públicos canônicos no package root.
  + camada de avaliação pura de regras cadastrais de materiais contra ground truth em memória (MaterialRulePrediction, MaterialRuleCaseEvaluation, MaterialRuleEvaluationReport, evaluate_material_rule e evaluate_material_rules) com pareamento determinístico por evaluation_case_id, validação relacional de material_id, exact set match (accuracy / exact_match_ratio), diagnósticos de true positives, false positives e false negatives, is_clean_match e is_defect_match, linhagem compulsória por dataset_id, semântica de dataset vazio (accuracy=None), ordenação canônica determinística por (evaluation_case_id, ground_truth_id), imutabilidade (frozen/slots), zero I/O e exports públicos canônicos no package root.
