...
- **Estado registrado em:** 2026-09-15
- **Baseline integrado na main:** 830 testes aprovados (100% GREEN)
- **Última entrega funcional integrada na main:** Ground Truth Decision Recommendation Evaluator v1
- **Última Issue funcional integrada na main:** #124
- **Último PR funcional integrado na main:** #126
- **Último merge funcional:** `05eed90` — Merge pull request #126
- **Última SPEC integrada na main:** `docs/specs/0124_ground_truth_decision_recommendation_evaluator_v1.md`
...
Fluxo conceitual de avaliação de recomendação:
...
Predictions Mapping
        ↓
pareamento por evaluation_case_id
        ↓
validação relacional por material_id
        ↓
DecisionRecommendationCaseEvaluation
        ↓
DecisionRecommendationEvaluationReport
...
- avaliar atômica e deterministicamente predições contra gabaritos de recomendação com validação de compatibilidade de `material_id` (`evaluate_decision_recommendation` → `DecisionRecommendationCaseEvaluation`);
- avaliar em lote mapeamentos explícitos contra datasets de recomendação (`evaluate_decision_recommendations` → `DecisionRecommendationEvaluationReport`), consumindo estritamente `Mapping[str, DecisionRecommendation]` indexado por `evaluation_case_id` com paridade exata 1:1 de chaves e validação relacional de `material_id`;
- garantir imutabilidade estrita (`frozen=True, slots=True`), ordenação canônica determinística por `(evaluation_case_id, ground_truth_id)`, semântica explícita de coleção vazia (`accuracy is None`), exact-match categórico e validações defensivas fail-closed;
...
MAIN INTEGRADA:
- Baseline integrado na main: 830 testes | unittest | Python 3.11.
- Última entrega funcional integrada na main: Issue #124 | Ground Truth Decision Recommendation Evaluator v1 | PR #126 (merge 05eed90).
- Última SPEC integrada: docs/specs/0124_ground_truth_decision_recommendation_evaluator_v1.md (Status: IMPLEMENTED).
- Último PR funcional integrado: PR #126.
- Último merge funcional: 05eed90.
