...
class DecisionRecommendationGroundTruthSerializationTests(unittest.TestCase):
    def _create_valid_record(
        self,
        *,
        provenance: str = LabelProvenance.SYNTHETIC_SPECIFIED.value,
        annotator: dict[str, object] | None = None,
        material_id: str = "MAT-100",
        ...
        return {
            ...
            "ground_truth_id": "GT-DR-001",
            "evaluation_case_id": "CASE-DR-001",
            "material_id": material_id,
            "expected_recommendation": expected_recommendation,
            "provenance": provenance,
            "source_reference": "SYNTH-SPEC-DR-V1",
            "annotator": annotator,
            "labeled_at": labeled_at.isoformat(),
            "rationale": "Synthetic decision recommendation ground truth",
        }
    ...
    def _create_valid_annotator(
        ...
        *,
        ...
        return {
            ...
            "identity_provider": "CORP_IDP",
            "identity_subject": "specialist.approver@corp.example",
            "verification_id": "VER-777",
            "verified_at": verified_at,
        }
    ...
    def test_round_trip_synthetic_specified(self) -> None:
        ...
        record = decision_recommendation_ground_truth_to_record(gt)
        ...
        self.assertEqual(record["ground_truth_id"], "GT-DR-001")
        self.assertEqual(record["evaluation_case_id"], "CASE-DR-001")
        self.assertEqual(record["material_id"], "MAT-100")
        self.assertEqual(
            record["expected_recommendation"],
            GovernanceDecision.APPROVE.value,
        )
        self.assertEqual(
            record["provenance"],
            LabelProvenance.SYNTHETIC_SPECIFIED.value,
        )
        self.assertEqual(record["source_reference"], "SYNTH-SPEC-DR-V1")
        self.assertIsNone(record["annotator"])
        self.assertEqual(record["labeled_at"], labeled_at.isoformat())
        self.assertEqual(
            record["rationale"],
            "Synthetic decision recommendation ground truth",
        )

        reconstructed = decision_recommendation_ground_truth_from_record(record)
        self.assertEqual(reconstructed, gt)
        self.assertEqual(reconstructed.material_id, "MAT-100")
        self.assertEqual(
            reconstructed.expected_recommendation,
    ...
    def test_round_trip_specialist_curated(self) -> None:
        ...
        gt = DecisionRecommendationGroundTruth(
            evaluation_case_id="CASE-DR-002",
            ground_truth_id="GT-DR-002",
            material_id="MAT-200",
            expected_recommendation=GovernanceDecision.REVIEW,
            provenance=LabelProvenance.SPECIALIST_CURATED,
            source_reference="CURATION-DR-2026",
            annotator=specialist,
            labeled_at=labeled_at,
            rationale="Specialist curation for review recommendation",
        )
        ...
        self.assertEqual(
            record["expected_recommendation"],
            GovernanceDecision.REVIEW.value,
        )
        self.assertEqual(
            record["provenance"],
            LabelProvenance.SPECIALIST_CURATED.value,
        ...
        self.assertIsInstance(record["annotator"], dict)
        annotator_record = record["annotator"]
        self.assertEqual(annotator_record["specialist_id"], "SPEC-003")
        self.assertEqual(annotator_record["identity_provider"], "CORP_IDP")
        self.assertEqual(
            annotator_record["identity_subject"],
            "specialist.approver@corp.example",
        )
        self.assertEqual(annotator_record["verification_id"], "VER-777")
        self.assertEqual(
            annotator_record["verified_at"],
            verified_at.isoformat(),
        )
    ...
    def test_governance_decision_preservation(self) -> None:
        decisions = [
            GovernanceDecision.APPROVE,
        ...
        for decision in decisions:
            with self.subTest(decision=decision.value):
                gt = DecisionRecommendationGroundTruth(
                    evaluation_case_id="CASE-DR-DEC",
                    ground_truth_id=f"GT-DR-{decision.value}",
                    material_id="MAT-DEC-01",
                    expected_recommendation=decision,
                    provenance=LabelProvenance.SYNTHETIC_SPECIFIED,
                    source_reference="SYNTH-SPEC-DEC",
                    annotator=None,
                    labeled_at=datetime(
                        2026, 9, 20, 10, 0, 0, tzinfo=timezone.utc
                    ),
                    rationale=f"Decision test for {decision.value}",
                )
                ...
                reconstructed = (
                    decision_recommendation_ground_truth_from_record(record)
                )
                self.assertEqual(
                    reconstructed.expected_recommendation,
                    decision,
    ...
    def test_material_id_preservation(self) -> None:
        material_id = "MAT-XYZ-001"
        gt = DecisionRecommendationGroundTruth(
            evaluation_case_id="CASE-DR-MAT",
            ground_truth_id="GT-DR-MAT-01",
            material_id=material_id,
            expected_recommendation=GovernanceDecision.APPROVE,
            provenance=LabelProvenance.SYNTHETIC_SPECIFIED,
            source_reference="SYNTH-SPEC-MAT",
            annotator=None,
            labeled_at=datetime(2026, 9, 20, 10, 0, 0, tzinfo=timezone.utc),
            rationale="Material ID preservation test",
        )

        record = decision_recommendation_ground_truth_to_record(gt)
        self.assertEqual(record["material_id"], material_id)

        reconstructed = decision_recommendation_ground_truth_from_record(record)
        self.assertEqual(reconstructed.material_id, material_id)
    ...
    def test_from_record_rejects_non_mapping_root(self) -> None:
        ...
        for invalid_input in invalid_inputs:
            with self.subTest(invalid_input=invalid_input):
                with self.assertRaises(ValueError):
                    decision_recommendation_ground_truth_from_record(invalid_input)  # type: ignore[arg-type]
    ...
    def test_from_record_rejects_invalid_schema_version(self) -> None:
        ...
        del record_missing["schema_version"]
        with self.assertRaises(ValueError):
            decision_recommendation_ground_truth_from_record(record_missing)
    ...
    # ==================================================
    # 5. expected_recommendation
    # ==================================================

    def test_from_record_rejects_invalid_expected_recommendation(self) -> None:
        ...
        for invalid_val in invalid_recommendations:
            with self.subTest(expected_recommendation=repr(invalid_val)):
                record = self._create_valid_record()
                record["expected_recommendation"] = invalid_val
                with self.assertRaises(ValueError):
                    decision_recommendation_ground_truth_from_record(record)
    ...
    def test_from_record_validates_temporal_relationship_between_verified_and_labeled(
        ...
        record_equal["labeled_at"] = same_instant
        obj = decision_recommendation_ground_truth_from_record(record_equal)
        self.assertIsNotNone(obj.annotator)
        self.assertEqual(obj.annotator.verified_at, obj.labeled_at)
    ...
    def test_to_record_rejects_non_decision_recommendation_ground_truth_instance(
        self,
    ) -> None:
        invalid_inputs = [None, {}, "invalid_string", 123, True]
        for invalid_input in invalid_inputs:
